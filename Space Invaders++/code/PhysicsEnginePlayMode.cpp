...
void PhysicsEnginePlayMode::detectInvaderCollisions(vector<GameObject>& objects, vector<int>& bulletPositions){
	...
	for(invaderIt; invaderIt != invaderEnd; invaderIt++){
		if((*invaderIt).isActive() && (*invaderIt).getTag() == "invader" ){
			...
			for(bulletIt; bulletIt != bulletEnd; bulletIt++){
				if((*invaderIt).getEncompassingRectCollider().intersects((*bulletIt)) && bulletIt.getTag() == "bullet" && static_pointer_cast<BulletUpdateComponent>((*bulletIt).getFirstUpdateComponent())->m_BelongsToPlayer){
...
void PhysicsEnginePlayMode::detectPlayerCollisionsAndInvaderDirection(vector<GameObject>& objects, vector<int>& bulletPositions){
	Vector2f offScreen(-1, -1);
	...
	for(it3; it3 != end3; it3++){
		if((*it3).isActive() && (*it3).hasCollider() && (*it).getTag() != "Player"){
			...
			//	Handle the direction and descent of the invader
